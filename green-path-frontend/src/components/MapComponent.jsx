...
import { User, Activity, Bike, Timer, Route, Loader2, RotateCcw } from "lucide-react";
...
function FitBounds({ routes }) {
	...
	useEffect(() => {
		...
		const bounds = routes.flatMap((r) => r.geometry.coordinates.map((c) => [c[1], c[0]]));
...
function MapComponent() {
  ...
  =========================== */
	...
	const speed = ageCategory && SPEED_TABLE[ageCategory][mode]; // km/h
						...
						>
							...
							{zones.map((zone, index) => (
								<Circle
									...
									radius={zone.radius}
									pathOptions={{
										color: zone.type === "INDUSTRIAL" ? "#ff0000" : "#ff9900",
										fillOpacity: 0.15,
									}}
								>
									<Popup>
							...
							<div className="flex flex-row justify-between gap-2 text-sm text-[#6B5FA7]">
								<div className="flex items-center gap-2">
									...
									<span className="capitalize">
										{ageCategory ? ageCategory : "Not Selected"}
									</span>
								</div>
